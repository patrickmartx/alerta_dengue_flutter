...
class InfoPage extends StatefulWidget {
  const InfoPage({super.key});import 'package:alerta_dengue/data/model_municipios.dart';
import 'package:alerta_dengue/main.dart';
...
class _InfoPageState extends State<InfoPage> {
  ...
  void didChangeDependencies() {
    ...
    if (!isInitialized) {
      ...
      estadoSelecionado = argumentos.sigla;
  ...
  Future<void> fetchCasosDengue(int codigoMunicipio) async {
    ...
    try {
      ...
      print("${e.toString()} erro aqui ${e.runtimeType} $e");
    }
  }
                        ...
                        onChanged: (Municipio? novoValor) {
                          setState(() {
                            ...
                            if (municipioSelecionado != null) {
                              fetchCasosDengue(municipioSelecionado!.codigoTexto);
                            }
                          });
                        },
                      ),
                  ...
                  : municipioSelecionado == null
                      ...
                      : CasosDengueCard(
                          parametros: getParametros(
                              data, nomeDoenca, municipioSelecionado!.codigoTexto),
                        ),
            ),
          ],
        ),
...
class CalcularSemanaEpidemiologica {
  static int getSemanaEpidemiologica(DateTime data) {
    DateTime primeiraSegundaFeiraDoAno = DateTime(data.year, 1, 1);
    while (primeiraSegundaFeiraDoAno.weekday != DateTime.monday) {
      primeiraSegundaFeiraDoAno = primeiraSegundaFeiraDoAno.add(const Duration(days: 1));
    }
    int diferencaEmDias = data.difference(primeiraSegundaFeiraDoAno).inDays;
...
String getParametros(DateTime date, String nomeDoenca, int codigoMunicipio) {
  int semanaEpidemiologica = CalcularSemanaEpidemiologica.getSemanaEpidemiologica(date);
  int ano = date.year;
  return "geocode=$codigoMunicipio&disease=${nomeDoenca.toLowerCase()}&format=json&ew_start=$semanaEpidemiologica&ey_start=$ano&ew_end=$semanaEpidemiologica&ey_end=$ano";
}
...
class _InfoPageState extends State<InfoPage> {
  ...
  Future<void> fetchMunicipios() async {
    try {
      final fetchedMunicipios = await ApiMunicipiosService.fetchMunicipios(estadoSelecionado);
      setState(() {
        municipios = fetchedMunicipios;
        isLoadingMunicipios = false;
      });
  ...
  Future<void> fetchCasosDengue(int codigoMunicipio) async {
    ...
    try {
      await ApiCasosDengueService.fetchCasosDengue(getParametros(data, nomeDoenca, codigoMunicipio));
      setState(() {
        isLoadingCasosDengue = false;
      });
    } catch (e) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text.rich(
          TextSpan(
            children: [
              TextSpan(text: "Alerta ", style: TextStyle(fontSize: 20.0, color: Colors.white)),
              TextSpan(text: "Dengue", style: TextStyle(fontSize: 20.0, color: Colors.red)),
            ],
          ),
        ),
        backgroundColor: Colors.black,
            ...
            Expanded(
              ...
              child: isLoadingCasosDengue
                  ...
                  : municipioSelecionado == null
                      ? const Center(child: Text('Selecione um município'))
                      : CasosDengueCard(
                          parametros: getParametros(data, nomeDoenca, municipioSelecionado!.codigoTexto),
                        ),
            ),
          ],
        ),
            ...
            return Card(
              ...
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('ID: ${casosDengue.id}', style: const TextStyle(fontWeight: FontWeight.bold)),
